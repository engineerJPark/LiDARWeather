...
class SemiBase3DSegmentor(Base3DSegmentor):
    ...
    def loss_by_pseudo_instances_masked_target(self, batch_inputs: dict,
                                 batch_data_samples: SampleList) -> dict:
        """Calculate losses from a batch of inputs and pseudo data samples.
        ...
        losses = self.student.decode_head.loss_by_feat(logits,
                                                       batch_data_samples)
        unsup_weight = self.semi_train_cfg.get('unsup_weight', 1.)
        losses = rename_loss_dict('unsup_masked_target_',
                                  reweight_loss_dict(losses, unsup_weight))
        return logits, losses

    def loss_by_pseudo_instances_masked_source(self, batch_inputs: dict,
                                 batch_data_samples: SampleList) -> dict:
        """Calculate losses from a batch of inputs and pseudo data samples.
        ...
        losses = self.student.decode_head.loss_by_feat(logits,
                                                       batch_data_samples)
        unsup_weight = self.semi_train_cfg.get('unsup_weight', 1.)
        losses = rename_loss_dict('unsup_masked_source_',
                                  reweight_loss_dict(losses, unsup_weight))
        return logits, losses
    ...
    def get_pseudo_instances_range_view(
            ...
            batch_data_samples: SampleList) -> Tuple[Tensor, SampleList]:
        """Get pseudo instances from teacher model."""
        logits = self.teacher(batch_inputs, batch_data_samples, mode='tensor') # logits[0]???
        # results_list = self.teacher.decode_head.predict_by_feat(
        #     logits, batch_data_samples)
