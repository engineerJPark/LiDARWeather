...
class LoadPointsFromFile(BaseTransform):
    ...
    def __init__(self,
                 ...
                 norm_intensity255: bool = False,
                 norm_elongation: bool = False,
                 norm_intensity_minmax: bool = False,
                 norm_intensity_sigmoid: bool = False,
        ...
        self.norm_intensity255 = norm_intensity255
        self.norm_intensity_minmax = norm_intensity_minmax
        self.norm_intensity_sigmoid = norm_intensity_sigmoid
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.norm_intensity:
            ...
            points[:, 3] = np.tanh(points[:, 3]) ## points[:, 3] = points[:, 3] / 255.
        ...
        if self.norm_intensity255:
            assert len(self.use_dim) >= 4, \
                f'When using intensity norm, expect used dimensions >= 4, got {len(self.use_dim)}'  # noqa: E501
            points[:, 3] = points[:, 3] / 255.
        if self.norm_intensity_minmax:
            assert len(self.use_dim) >= 4, \
                f'When using intensity norm, expect used dimensions >= 4, got {len(self.use_dim)}'  # noqa: E501
            points[:, 3] = (points[:, 3] - np.min(points[:, 3])) / (np.max(points[:, 3]) - np.min(points[:, 3]))
        if self.norm_intensity_sigmoid:
            assert len(self.use_dim) >= 4, \
                f'When using intensity norm, expect used dimensions >= 4, got {len(self.use_dim)}'  # noqa: E501
            points[:, 3] = sigmoid(points[:, 3])
        attribute_dims = None
