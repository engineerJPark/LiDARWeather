...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    ### newly added
    def _draw_pts_gt_pred(self,
                          points: Union[Tensor, np.ndarray],
                          pts_seg_pred: PointData,
                          pts_seg_gt: PointData,
                          keep_index: Optional[int] = None) -> None:
        """Draw 3D semantic mask of GT or prediction.
        ...
        Args:
            ...
            ignore_index (int, optional): Ignore category. Defaults to None.
        """
        # assert seg_mask_colors.shape[0] == gt_labels.shape[0] == pred_labels.shape[0], "Point, GT, and Pred arrays must have the same length"
        correct_prediction_color = np.array([128, 128, 128])  # Gray color
        incorrect_prediction_color = np.array([255, 0, 0])  # Red color
        check_type('points', points, (np.ndarray, Tensor))

        points = tensor2ndarray(points)
        pts_sem_seg_pred = tensor2ndarray(pts_seg_pred.pts_semantic_mask)
        pts_sem_seg_gt = tensor2ndarray(pts_seg_gt.pts_semantic_mask)

        if keep_index is not None:
            keep_index = tensor2ndarray(keep_index)
            points = points[keep_index]
            pts_sem_seg_pred = pts_sem_seg_pred[keep_index]
            pts_sem_seg_gt = pts_sem_seg_gt[keep_index]

        # Assign colors based on correct/incorrect predictions
        seg_colors = np.zeros_like(points[:, 3:])
        correct_predictions = pts_sem_seg_gt == pts_sem_seg_pred
        seg_colors[correct_predictions] = correct_prediction_color
        seg_colors[~correct_predictions] = incorrect_prediction_color

        # Combine points with their colors
        seg_color = np.concatenate([points[:, :3], seg_colors], axis=1)
        self.draw_seg_mask(seg_color)
        ...
        if draw_pred and data_sample is not None:
            ...
            if 'pred_pts_seg' in data_sample and vis_task == 'lidar_seg':
                ...
                self._draw_pts_sem_seg(data_input['points'],
                                       ...
                                       keep_index)
            ### newly added
            if 'gt_pts_seg' in data_sample and vis_task == 'lidar_seg_gt_pred':
                assert classes is not None, 'class information is ' \
                                            'not provided when ' \
                                            'visualizing semantic ' \
                                            'segmentation results.'
                assert 'points' in data_input
                self._draw_pts_gt_pred(data_input['points'],
                                       data_sample.pred_pts_seg, data_sample.gt_pts_seg,
                                       keep_index)
