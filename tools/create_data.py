...
from tools.dataset_converters import stf_converter as stf
...
from tools.dataset_converters import nuscenes_c_converter as nuscenes_c_converter
from tools.dataset_converters import semantickitti_converter
from tools.dataset_converters import semantickitti_c_converter
from tools.dataset_converters import semanticstf_converter
from tools.dataset_converters import synlidar_converter
...
def nuscenes_c_data_prep(root_path,
                       info_prefix,
                       version,
                       dataset_name,
                       out_dir,
    ...
    Args:
        ...
        max_sweeps (int, optional): Number of input consecutive frames.
            Default: 10
    """
    nuscenes_c_converter.create_nuscenes_infos(
        root_path, info_prefix, version=version, max_sweeps=max_sweeps)
    ...
    info_train_path = osp.join(out_dir, f'{info_prefix}_infos_train.pkl')
    info_val_path = osp.join(out_dir, f'{info_prefix}_infos_val.pkl')
    update_pkl_infos('nuscenes', out_dir=out_dir, pkl_path=info_train_path) ## Error
    update_pkl_infos('nuscenes', out_dir=out_dir, pkl_path=info_val_path)
    create_groundtruth_database(dataset_name, root_path, info_prefix,
                                f'{info_prefix}_infos_train.pkl')
...
def semantickitti_c_data_prep(info_prefix, out_dir):
    """Prepare the info file for SemanticKITTI-C dataset.

    Args:
        info_prefix (str): The prefix of info filenames.
        out_dir (str): Output directory of the generated info file.
    """
    semantickitti_c_converter.create_semantickitti_info_file(
        info_prefix, out_dir)

def semanticstf_data_prep(info_prefix, out_dir):
    """Prepare the info file for SemanticSTF dataset.

    Args:
        info_prefix (str): The prefix of info filenames.
        out_dir (str): Output directory of the generated info file.
    """
    semanticstf_converter.create_semantickitti_info_file(
        info_prefix, out_dir)

def synlidar_data_prep(info_prefix, out_dir):
    """Prepare the info file for SynLiDAR dataset.

    Args:
        info_prefix (str): The prefix of info filenames.
        out_dir (str): Output directory of the generated info file.
    """
    synlidar_converter.create_synlidar_info_file(
        info_prefix, out_dir)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'kitti':
        ...
        if args.only_gt_database:
            ...
            nuscenes_data_prep(
                ...
                out_dir=args.out_dir,
                max_sweeps=args.max_sweeps)
    elif args.dataset == 'nuscenes_c' and args.version != 'v1.0-mini':
        if args.only_gt_database:
            create_groundtruth_database('NuScenes_C_Dataset', args.root_path,
                                        args.extra_tag,
                                        f'{args.extra_tag}_infos_train.pkl')
        else:
            train_version = f'{args.version}-trainval'
            nuscenes_c_data_prep(
                root_path=args.root_path,
                info_prefix=args.extra_tag,
                version=train_version,
                dataset_name='NuScenes_C_Dataset',
                out_dir=args.out_dir,
                max_sweeps=args.max_sweeps)
            test_version = f'{args.version}-test'
            nuscenes_c_data_prep(
    ...
    elif args.dataset == 'semantickitti_c':
        semantickitti_c_data_prep(
            info_prefix=args.extra_tag, out_dir=args.out_dir)
    elif args.dataset == 'semanticstf':
        semanticstf_data_prep(
            info_prefix=args.extra_tag, out_dir=args.out_dir)
    elif args.dataset == 'synlidar':
        synlidar_data_prep(
            info_prefix=args.extra_tag, out_dir=args.out_dir)
    else:
