...
def set_split(imageset_folder, split):
    # root_split_path = self.root_path / ('train' if self.split != 'test' else 'test')
    split_dir = imageset_folder / f'{split}.txt'

    # pdb.set_trace()
    if split_dir.exists():
        sample_id_list = ['_'.join(x.strip().split(',')) for x in open(split_dir).readlines()]
    else:
        sample_id_list = None
    return sample_id_list

def create_stf_info_file(data_path,
                           pkl_prefix='stf',
                           with_plane=False,
                           save_path=None,
                           relative_path=False,
                           ...
                           val: bool=True,
                           test: bool=True,):
    """Create info file of STF dataset.
    ...
    num_samples = {
        ...
        "snow_day" : 2293,
        "snow_night" : 2440,
        "test_clear_day" : 1005, # "test_clear_day" : 1005,
        "test_clear_night" : 877, # "test_clear_night" : 877,
        "val_clear_day" : 399, # "val_clear_day" : 399,
        "val_clear_night" : 409, # "val_clear_night" : 409,
        "train_clear_day" : 2183, # "train_clear_day" : 2183,
        "train_clear_night" : 1343, # "train_clear_night" : 1343,
    }
    imageset_folder = Path(data_path) / 'splits'
    ...
    for time in ['day', 'night']:
        print(f'starting to process {time}time scenes')
        # logger.info(f'starting to process {time}time scenes')

        train_split, train_filename = None, None
        val_split, val_filename = None, None
        test_split, test_filename = None, None
        # dense_infos_train, dense_infos_val = None, None

        # train split
        if train:
            train_split = f'train_{time}'
            train_filename = save_path / f'stf_infos_{train_split}.pkl'

            sample_id_list = set_split(imageset_folder, train_split)
            kitti_infos_train = get_stf_image_info(
                data_path,
                training=True,
                velodyne=True,
                calib=True,
                image_ids=sample_id_list,
                relative_path=relative_path)
            _calculate_num_points_in_gt(data_path, kitti_infos_train, relative_path, num_features=5)
            print(f'Kitti info train file is saved to {train_filename}')
            mmengine.dump(kitti_infos_train, train_filename)

        # val split
        if val:
            val_split = f'val_{time}'
            val_filename = save_path / f'stf_infos_{val_split}.pkl'

            sample_id_list = set_split(imageset_folder, val_split)
            kitti_infos_val = get_stf_image_info(
                data_path,
                training=True,
                velodyne=True,
                calib=True,
                image_ids=sample_id_list,
                relative_path=relative_path)
            _calculate_num_points_in_gt(data_path, kitti_infos_val, relative_path, num_features=5)
            print(f'Kitti info train file is saved to {val_filename}')
            mmengine.dump(kitti_infos_val, val_filename)

        # trainval concatination
        if train and val:
            trainval_filename = save_path / f'stf_infos_trainval_{time}.pkl'
            print(f'Kitti info train file is saved to {trainval_filename}')
            mmengine.dump(kitti_infos_train + kitti_infos_val, trainval_filename)


        # test splits
        ## 조건 보완이 필요해보인다. test
        if test:
            for condition in ['clear', 'light_fog', 'dense_fog', 'snow', 'rain']:
                # for alpha in [0.45]:
                #     for severety in ['none', 'light', 'heavy']:
                if condition == 'clear':
                    test_split = f'test_{time}'
                elif condition == 'rain':
                    test_split = f'{condition}'
                else:
                    test_split = f'{condition}_{time}'
                test_filename = save_path / f'stf_infos_{test_split}.pkl'
                sample_id_list = set_split(imageset_folder, test_split)

                if sample_id_list is None:
                    print(f'{test_split} does not exist')
                    continue

                kitti_infos_test = get_stf_image_info(
                    data_path,
                    training=True,
                    velodyne=True,
                    calib=True,
    ...
    for stage in ['train', 'val', 'trainval']:
        ...
        save_file = f'{pkl_dir}/stf_infos_{stage}.pkl'
        day_file = f'{pkl_dir}/stf_infos_{stage}_day.pkl'
        night_file = f'{pkl_dir}/stf_infos_{stage}_night.pkl'
    ...
    if test:
        for condition in ['clear', 'light_fog', 'dense_fog', 'snow', 'rain']:
            ...
            #     for severety in ['none', 'light', 'heavy']:
            if condition == 'clear':
                # test_split = f'test_{condition}_{time}'
                save_file = f'{pkl_dir}/stf_infos_test.pkl'
                day_file = f'{pkl_dir}/stf_infos_test_day.pkl'
                night_file = f'{pkl_dir}/stf_infos_test_night.pkl'
            elif condition == 'rain':
                # test_split = f'{condition}'
                save_file = f'{pkl_dir}/stf_infos_{condition}.pkl'
                day_file = f'{pkl_dir}/stf_infos_{condition}_day.pkl'
                night_file = f'{pkl_dir}/stf_infos_{condition}_night.pkl'
            else:
                # test_split = f'{condition}_{time}'
                save_file = f'{pkl_dir}/stf_infos_{condition}.pkl'
                day_file = f'{pkl_dir}/stf_infos_{condition}_day.pkl'
                night_file = f'{pkl_dir}/stf_infos_{condition}_night.pkl'
            # save_file = f'{pkl_dir}/stf_infos_test_{condition}.pkl'
            # day_file = f'{pkl_dir}/stf_infos_test_{condition}_day.pkl'
            # night_file = f'{pkl_dir}/stf_infos_test_{condition}_night.pkl'
...
def export_2d_annotation(root_path, info_path, mono3d=True):
    ...
    kitti_infos = mmengine.load(info_path)
    cat2Ids = [
        dict(id=stf_categories.index(cat_name), name=cat_name)
        for cat_name in stf_categories
    ]
    coco_ann_id = 0
...
def generate_record(ann_rec, x1, y1, x2, y2, sample_data_token, filename):
    ...
    if repro_rec['category_name'] not in stf_categories:
        return None
    cat_name = repro_rec['category_name']
    coco_rec['category_name'] = cat_name
    coco_rec['category_id'] = stf_categories.index(cat_name)
