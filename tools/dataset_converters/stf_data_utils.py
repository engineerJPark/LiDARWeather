...
DENSE_TO_STF_LABEL={
    ...
    'RidableVehicle_is_group':'Cyclist',
    'LargeVehicle':'Van',
    'LargeVehicle_is_group':'Van',
    'Vehicle':'Van',
    'Vehicle_is_group':'Van',
    'DontCare':'DontCare',
    'Obstacle':'DontCare',
    'train':'DontCare',
    }
...
def get_label_path(idx,
                   ...
                   relative_path=True,
                   exist_check=True,
                   info_type='cam_left_labels_TMP', # gated_labels_TMP
                   use_prefix_id=False):
...
def get_velodyne_path(idx,
    ...
    return get_kitti_info_path(idx, prefix, 'lidar_hdl64_last', '.bin', training,
                            relative_path, exist_check, use_prefix_id)
...
def get_label_anno(label_path):
    ...
    num_objects = len([DENSE_TO_STF_LABEL[x[0]] for x in content if x[0] != 'DontCare'])
    annotations['name'] = np.array([DENSE_TO_STF_LABEL[x[0]] for x in content])
